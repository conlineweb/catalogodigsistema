...
startSession();
if (isLoggedIn()) { header('Location: /catalogodigsistema/client/index.php'); exit; }
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<title>Mi Pedido · AutoRepuestos Pro</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Inter',sans-serif;background:linear-gradient(135deg,#1a2c3e 0%,#2c4b57 60%,#1f3e48 100%);min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:1rem}
...
.brand h1{font-size:1.5rem;font-weight:800;color:#1a2c3e}
...
.btn{width:100%;padding:.95rem;background:linear-gradient(135deg,#C0392B,#E67E22);border:none;border-radius:60px;color:#fff;font-weight:700;font-size:1rem;cursor:pointer;font-family:'Inter',sans-serif;transition:.2s}
...
.hint{text-align:center;margin-top:1.2rem;font-size:.75rem;color:#aaa}
.hint a{color:#E67E22;text-decoration:none}
.info-box{background:#f0f9ff;border:1px solid #bae6fd;border-radius:16px;padding:1rem;font-size:.83rem;color:#0369a1;margin-bottom:1.4rem;line-height:1.6}
.info-box i{color:#0284c7;margin-right:4px}
</style>
</head>
<body>
<div class="card">
  ...
  <div class="info-box">
    <i class="fas fa-info-circle"></i> Usa el <strong>usuario</strong> y <strong>contraseña temporal</strong> que recibiste al realizar tu pedido.
  </div>
  <div class="alert" id="alertMsg"></div>
  <form id="loginForm" novalidate>
    <div class="field">
      <label><i class="fas fa-user fa-sm"></i> Usuario</label>
      <input type="text" id="uname" placeholder="Ej: juan001" autocomplete="username" required>
    </div>
    <div class="field">
    ...
    <button type="submit" class="btn" id="loginBtn"><i class="fas fa-search"></i> Ver mi pedido</button>
  </form>
  <p class="hint"><a href="/catalogodigsistema/index.php">← Volver a la tienda</a></p>
</div>
<script>
document.getElementById('loginForm').addEventListener('submit', async e => {
  ...
  try {
    ...
    if (data.success) {
      window.location = '/catalogodigsistema/client/index.php';
    } else {
      alert.textContent = data.error || 'Credenciales incorrectas';
      alert.className   = 'alert show';
      btn.disabled = false;
      btn.innerHTML = '<i class="fas fa-search"></i> Ver mi pedido';
    }
  } catch {
